...
# این خط را حذف یا کامنت کنید تا هر اکانتی بتواند استفاده کند
...
class SinglCMD(Plugin):
    def __init__(s):
        ...
        # قسمت چک اکانت حذف شد تا هر اکانتی بتواند استفاده کند
    ...
    def ear(s):
        ...
        # آنتی‌کد فقط زمانی کار می‌کند که:
        # 1. آنتی‌کد فعال باشد
        # 2. فرستنده پیام OWNER نباشد (یعنی افراد عادی به ادمین کد می‌زنند)
        ...
        if anti_enabled and not is_owner:
            ...
            if anti_cmds and owner_client_id and owner_client_id != 'Not set':
                ...
                if m_lower.startswith('%'):
                    ...
                    # بررسی اینکه آیا کد در لیست آنتی‌کد وجود دارد و target_id برابر owner_id است
        ...
        # کدهای معمولی فقط برای OWNER کار می‌کند
        if not is_owner:
            return  # اگر OWNER نباشد، کدهای معمولی را نادیده بگیر
                        ...
                        for p in client['players']:
                            if p.get('name', '') == sender:
                                ...
                                if owner_account.lower() in device_name.lower():
                                    # به‌روزرسانی nickname در صورت نیاز
