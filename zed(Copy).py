# SinglCMD - Server Code System (نسخه عمومی با قابلیت ON/OFF)
...
# متغیرهای کنترل دسترسی عمومی
PUBLIC_MODE = True  # True = همه می‌توانند استفاده کنند، False = فقط مالک
PLUGIN_ENABLED = True  # True = پلاگین فعال، False = غیرفعال
...
class SC:
    ...
    def __init__(s, source: bw = None) -> None:
        ...
        w = s.w = SC.cw(
            source=source,
            size=(250, 600),  # افزایش ارتفاع برای دکمه ON/OFF
        )
        scroll_height = 490
        scroll_width = 230
        scroll = sw(
        ...
        # ===== دکمه ON/OFF =====
        # دریافت وضعیت فعلی
        plugin_status = var('plugin_enabled')
        if plugin_status is None:
            plugin_status = True
            var('plugin_enabled', True)
        
        # دکمه ON/OFF در بالای منو
        status_text = "🔴 OFF" if not plugin_status else "🟢 ON"
        status_color = (1, 0.3, 0.3) if not plugin_status else (0.3, 1, 0.3)
        ...
        on_off_btn = SC.bw(
            ...
            size=(100, 30),
            position=(75, 545),
            on_activate_call=CallStrict(s._toggle_plugin),
            color=(0.15, 0.15, 0.25),
            textcolor=status_color
        )
        
        # نمایش وضعیت در پایین منو
        owner_account = var('owner_account')
        owner_nickname = var('owner_nickname') or owner_account
        owner_client_id = var('owner_client_id') or ''
        
        # وضعیت عمومی/خصوصی
        public_mode = var('public_mode')
        if public_mode is None:
            public_mode = True
            var('public_mode', True)
        
        mode_text = "🌐 Public" if public_mode else "🔒 Private"
        tw(
            parent=w,
            text=mode_text,
            scale=0.6,
            position=(125, 25),
            h_align='center',
            color=(0.6, 1, 0.6) if public_mode else (1, 0.8, 0.4)
        )
    ...
    def _toggle_plugin(s):
        """تغییر وضعیت ON/OFF پلاگین"""
        current_status = var('plugin_enabled')
        if current_status is None:
            current_status = True
        
        new_status = not current_status
        var('plugin_enabled', new_status)
        
        if new_status:
            push('✅ SinglCMD Activated!', color=(0, 1, 0))
            gs('dingSmallHigh').play()
        else:
            push('❌ SinglCMD Deactivated!', color=(1, 0.5, 0))
            gs('block').play()
        
        # بازسازی منو برای به‌روزرسانی دکمه
        if hasattr(s, 'w') and s.w:
            source = None
            for child in s.w.get_children():
                if hasattr(child, 'get_screen_space_center'):
                    source = child
                    break
            SC.swish(s.w)
            teck(0.1, CallStrict(s.__init__, source))
...
# تنظیم مقادیر پیش‌فرض برای متغیرهای جدید
if var('plugin_enabled') is None:
    var('plugin_enabled', True)
if var('public_mode') is None:
    var('public_mode', True)
...
class SinglCMD(Plugin):
    def __init__(s):
        ...
        # حذف چک MASTER_ACCOUNT - همه می‌توانند استفاده کنند
        # اما همچنان اطلاعات نمایش داده می‌شود
        print(f"SinglCMD: Plugin loaded for account: {current_account}")
        print(f"SinglCMD: Public Mode: {var('public_mode')}")
        print(f"SinglCMD: Plugin Enabled: {var('plugin_enabled')}")
    ...
    def is_owner(s, sender):
        """بررسی مالکیت با پشتیبانی از حالت عمومی"""
        # اگر پلاگین غیرفعال است، هیچ کاری نکن
        if not var('plugin_enabled'):
            return False
        
        # اگر حالت عمومی فعال است، همه مالک هستند
        if var('public_mode'):
            return True
        
        # حالت خصوصی - فقط مالک واقعی
        owner_account = var('owner_account')
        owner_nickname = var('owner_nickname')
        
        if not owner_account and not owner_nickname:
            return True  # اگر مالک تنظیم نشده، همه می‌توانند
    ...
    def ear(s):
        # اگر پلاگین غیرفعال است، گوش نده
        if not var('plugin_enabled'):
