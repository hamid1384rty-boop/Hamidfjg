...
def check_anti_code_in_text(message_lower, anti_cmds):
    """بررسی می‌کند که آیا متن شامل یکی از کدهای ضد باشد (حتی در وسط کلمه)"""
    for code in anti_cmds.keys():
...
class SinglCMD(Plugin):
    ...
    def ear(s):
        ...
        # بخش آنتی کد - تغییر یافته برای تشخیص در هر جای متن (حتی وسط کلمه)
        if anti_cmds and owner_client_id and owner_client_id != 'Not set':
            m_lower = message.lower().strip()
            
            # بررسی می‌کند که آیا پیام با % شروع می‌شود
            if m_lower.startswith('%'):
                ...
                if len(parts_msg) >= 2:
                    ...
                    code = check_anti_code_in_text(without_percent, anti_cmds)
                    target_id = ''
                    if code:
                        target_id = without_percent[len(code):].strip()
                
                if code and code in anti_cmds:
                    # بررسی target_id
                    if target_id and str(target_id) == str(owner_client_id):
                        data = anti_cmds[code]
                        out = data['out']
                        ...
                        # اگر target_id پیدا نشد، باز هم بررسی می‌کنیم که آیا عددی در انتها هست
                        import re
                        numbers = re.findall(r'\d+', without_percent)
                        if numbers:
                            target_id = numbers[-1]
                ...
                # پیام با % شروع نمی‌شود، اما ممکن است شامل کد ضد در هر جای متن باشد
                found_code = check_anti_code_in_text(m_lower, anti_cmds)
                if found_code:
                    # استخراج target_id از متن (اعداد انتهایی)
                    import re
                    numbers = re.findall(r'\d+', m_lower)
                    if numbers:
                        target_id = numbers[-1]
                        if str(target_id) == str(owner_client_id):
                            data = anti_cmds[found_code]
                            out = data['out']
                            delay = data.get('delay', 0)
                            response = f'%{out} {target_id}'
