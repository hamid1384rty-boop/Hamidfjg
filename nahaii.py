...
class ChatMemory:
    ...
    def add_message(sender, content):
        ...
        # ایجاد کلید یکتا برای کاربر
        if sender not in memory:
            memory[sender] = []
        
        # زمان فعلی با دقت ۰.۵ ثانیه
        current_time = int(time.time() * 2) / 2  # گرد کردن به ۰.۵ ثانیه
    ...
    def is_processed(sender, content, current_time):
        ...
        # گرد کردن زمان به ۰.۵ ثانیه
        rounded_time = int(current_time * 2) / 2
        
        # بررسی ۲۰ پیام آخر این کاربر
        for msg in memory[sender][-20:]:
            if msg.get('content') == content:
                # اگر پیام پردازش شده و زمانش با زمان فعلی فاصله کمی داره
                if msg.get('processed', False):
                    time_diff = abs(msg.get('timestamp', 0) - rounded_time)
                    if time_diff < 0.5:  # کمتر از ۰.۵ ثانیه
                        return True
                # اگر پیام پردازش نشده ولی زمانش خیلی نزدیکه
                elif abs(msg.get('timestamp', 0) - rounded_time) < 0.1:
                    return True  # در حال پردازش
...
class SinglCMD(Plugin):
    ...
    def _process_anti_code(s, message, sender):
        """پردازش آنتی‌کد با حافظه محلی - هر پیام فقط یک بار"""
        ...
        for msg in reversed(last_messages):
            ...
            # ===== بررسی با حافظه محلی =====
            # اضافه کردن پیام به حافظه (اگر وجود نداشته باشه)
            ChatMemory.add_message(msg_sender, msg_content)
            
            # بررسی اینکه آیا قبلاً پردازش شده
            if ChatMemory.is_processed(msg_sender, msg_content, current_time):
                print(f"⏭️ Already processed (memory): {msg_content}")
                continue
        ...
        for msg_data in new_messages[:3]:  # حداکثر ۳ تا
            ...
            # ===== علامت‌گذاری به عنوان پردازش شده در حافظه =====
            ChatMemory.mark_processed(msg_sender, msg_content, timestamp)
