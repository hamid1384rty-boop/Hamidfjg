...
class SinglCMD(Plugin):
    ...
    def _process_anti_code(s, message, sender):
        ...
        # اگر کاربر مالک باشد، آنتی‌کد اجرا نمی‌شه
        if is_owner:
            print(f"⛔ {sender} is owner, skipping anti-code")
            return False
        ...
        print(f"🔍 Anti-code check: sender={sender}, is_owner={is_owner}, message={message}")
        ...
        if len(parts) >= 2:
            ...
            print(f"🔍 Method 1: code='{code}', target_id='{target_id}'")
        else:
            ...
            for anti_code in anti_cmds.keys():
                if without_percent.startswith(anti_code):
                    ...
                    print(f"🔍 Method 2: code='{code}', target_id='{target_id}'")
                    break
            ...
            if not code:
                ...
                if without_percent in anti_cmds:
                    ...
                    target_id = ''
                    print(f"🔍 Method 3: code='{code}', target_id='{target_id}'")
        ...
        if code and code in anti_cmds:
            ...
            if not target_id:
                ...
                print(f"🔍 Using default target_id: {target_id}")
            ...
            if str(target_id) == str(owner_client_id):
                ...
                print(f"❌ Target ID mismatch: {target_id} != {owner_client_id}")
        else:
            print(f"❌ Code not found: code='{code}' in {anti_cmds.keys()}")
