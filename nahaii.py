...
class AntiCodeNumber:
    def __init__(s, source):
        ...
        # نمایش مقدار فعلی
        current_count = var('anti_check_count')
        if current_count is None:
            current_count = 4
            var('anti_check_count', 4)
        
        tw(
            parent=w,
            text=f'مقدار فعلی: {current_count} پیام آخر',
            scale=0.8,
            position=(150, 145),
            h_align='center',
            color=(0.8, 1, 0.8)
        ...
        for i in range(1, 5):
            x_pos = start_x + (i - 1) * (button_width + 10)
            SC.bw(
                parent=w,
                label=str(i),
                size=(button_width, button_height),
                position=(x_pos, y_pos),
                on_activate_call=CallStrict(s._set_number, i),
                color=(0.2, 0.3, 0.4) if i != current_count else (0.2, 0.5, 0.2)
            )
        
        # دکمه تنظیم به ۴ (پیش‌فرض)
        SC.bw(
            parent=w,
            label='↺ Reset (4)',
            size=(100, 30),
            position=(100, 20),
            on_activate_call=CallStrict(s._set_number, 4),
            color=(0.1, 0.3, 0.1)
        )
        
        SC.swish()
    
    def _set_number(s, num):
        """تنظیم تعداد پیام‌های بررسی"""
        var('anti_check_count', num)
        SC.ok()
        push(f'📊 تعداد پیام‌های بررسی به {num} پیام آخر تغییر یافت', color=(0, 1, 0))
...
class AntiCodeMenu:
    def __init__(s, source):
        ...
        # دکمه تعداد پیام‌های بررسی
        SC.bw(
            parent=w,
            label='🔢 Number (پیام‌های بررسی)',
            size=(180, 35),
            position=(60, 325),
            on_activate_call=CallPartial(AntiCodeNumber, w),
            color=(0.1, 0.2, 0.4)
        )
        ...
        tw(
            parent=w,
            text=f'بررسی: {current_count} پیام آخر',
            scale=0.6,
            position=(135, 300),
            h_align='center',
            color=(0.8, 0.8, 1)
...
# مقدار پیش‌فرض برای تعداد پیام‌های بررسی
if var('anti_check_count') is None:
    var('anti_check_count', 4)
...
class SinglCMD(Plugin):
    ...
    def _extract_code_and_target(self, message, cmds_dict):
        ...
        if len(parts) >= 2:
            ...
            print(f"🔍 Method 1 (space): code='{code}', target_id='{target_id}'")
            return code, target_id
        
        # روش ۲: بدون فاصله (مثلاً %kick129 یا %kick 129 بدون فاصله)
        ...
        for cmd_code in sorted_codes:
            if without_percent.startswith(cmd_code):
                ...
                # اگر بعد از کد چیزی باقی مونده، اون رو target_id در نظر بگیر
                if remaining:
                    target_id = remaining
                else:
                    target_id = ''
                print(f"🔍 Method 2 (no space): code='{code}', target_id='{target_id}'")
                return code, target_id
        ...
        if without_percent in cmds_dict:
            ...
            target_id = ''
            print(f"🔍 Method 3 (exact match): code='{code}', target_id='{target_id}'")
            return code, target_id
        
        # روش ۴: اگر کد با اعداد ترکیب شده بود (مثلاً kick129)
        # این روش برای مواردی که کد با عدد شروع میشه یا ترکیب خاصی داره
        for cmd_code in sorted_codes:
            # بررسی اینکه آیا کد در ابتدای متن هست (با حروف کوچک)
            if m_lower.startswith('%' + cmd_code):
                code = cmd_code
                remaining = m_lower[len('%' + cmd_code):].strip()
                if remaining:
                    target_id = remaining
                else:
                    target_id = ''
                print(f"🔍 Method 4 (starts with): code='{code}', target_id='{target_id}'")
                return code, target_id
    ...
    def _process_anti_code(s, message, sender):
        """پردازش آنتی‌کد - برای همه کاربران با پشتیبانی از تمام فرمت‌ها و اسکن چند پیام آخر"""
        ...
        # دریافت چند پیام آخر (تعداد از متغیر تنظیم شده)
        all_messages = GCM()
        if not all_messages:
            return False
        
        # دریافت تعداد از تنظیمات
        check_count = var('anti_check_count')
        if check_count is None:
            check_count = 4
            var('anti_check_count', 4)
        
        # فقط تعداد مشخصی پیام آخر رو بررسی کن
        last_messages = all_messages[-check_count:] if len(all_messages) >= check_count else all_messages
        ...
        for msg in last_messages:
            ...
            # اگر پیام از خود sender نباشه یا پیام تکراری باشه، رد کن
            if msg_sender != sender:
                continue
            
            # اگر پیام در لیست ignore باشه، رد کن
            if msg_content in s.ignore_messages:
                continue
            ...
            # بررسی تطابق با owner_client_id
            if str(target_id) == str(owner_client_id):
                ...
                print(f"✅ Anti-code triggered: {msg_sender} -> {code} -> {response}")
                ...
                # اگر ۲ تا کد مختلف پیدا شد، باز هم ادامه بده
    ...
    def ear(s):
        ...
        # ===== اول آنتی‌کد رو بررسی کن (برای همه کاربران، حتی مالک) =====
