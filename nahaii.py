...
class SinglCMD(Plugin):
    def __init__(s):
        ...
        s.z = []
        s.ignore_messages = []
        # لیست برای ذخیره پیام‌های آنتی‌کد پردازش شده
        s._processed_anti_messages = []
        s._last_sender = None
    ...
    def _process_anti_code(s, message, sender):
        """پردازش آنتی‌کد با پیگیری کدهای پردازش شده"""
        ...
        for msg in reversed(last_messages):  # از جدید به قدیم
            if ': ' not in msg:
                continue
            ...
            # چک کردن اینکه این پیام قبلاً پردازش شده
            msg_hash = f"{msg_sender}:{msg_content}"
            if msg_hash in s._processed_anti_messages:
                print(f"⏭️ Skipping already processed message: {msg_content}")
                continue
            ...
            if str(target_id) == str(owner_client_id):
                ...
                s._processed_anti_messages.append(msg_hash)
                
                # محدود کردن لیست برای جلوگیری از بزرگ شدن
                if len(s._processed_anti_messages) > 20:
                    s._processed_anti_messages = s._processed_anti_messages[-10:]
    ...
    def ear(s):
        ...
        if hasattr(s, '_last_sender') and s._last_sender != sender:
            s._processed_anti_messages = []
            print(f"🔄 Sender changed from {s._last_sender} to {sender}, cleared processed list")
