...
class SinglCMD(Plugin):
    def __init__(s):
        ...
        s.z = []
        s.ignore_messages = []
        # تاریخچه ارسال با دقت بالا
        s._send_history = {}  # {sender: [(content, timestamp, processed), ...]}
        # کش پردازش شده
        s._processed_anti_cache = {}  # {sender: [unique_ids, ...]}
        s._last_sender = None
        s._clean_counter = 0
    ...
    def _clean_old_history(s):
        """پاک کردن تاریخچه ارسال قدیمی (بیش از ۶۰ ثانیه)"""
        if not hasattr(s, '_send_history'):
            return
        
        current_time = time.time()
        
        for sender in list(s._send_history.keys()):
            # حذف پیام‌های قدیمی (بیش از ۶۰ ثانیه)
            old_messages = []
            for content, timestamp, processed in s._send_history[sender]:
                if current_time - timestamp > 60:  # بیشتر از ۶۰ ثانیه
                    old_messages.append((content, timestamp, processed))
            
            for old_msg in old_messages:
                s._send_history[sender].remove(old_msg)
            
            # اگر لیست کاربر خالی شد، حذفش کن
            if not s._send_history[sender]:
                del s._send_history[sender]
                print(f"🗑️ Removed empty history for {sender}")
    
    def _clean_old_cache(s):
        """پاک کردن کش پردازش قدیمی (بیش از ۶۰ ثانیه)"""
        if not hasattr(s, '_processed_anti_cache'):
            return
        
        current_time = time.time()
        
        for sender in list(s._processed_anti_cache.keys()):
            # حذف شناسه‌های قدیمی (بیش از ۶۰ ثانیه)
            old_ids = []
            for unique_id in s._processed_anti_cache[sender]:
                parts = unique_id.split(':')
                if len(parts) >= 3:
                    try:
                        timestamp = float(parts[2])
                        if current_time - timestamp > 60:  # بیشتر از ۶۰ ثانیه
                            old_ids.append(unique_id)
                    except:
                        pass
            
            for old_id in old_ids:
                s._processed_anti_cache[sender].remove(old_id)
            
            # اگر لیست کاربر خالی شد، حذفش کن
            if not s._processed_anti_cache[sender]:
                del s._processed_anti_cache[sender]
                print(f"🗑️ Removed empty cache for {sender}")
    ...
    def _process_anti_code(s, message, sender):
        """پردازش آنتی‌کد با تاریخچه دقیق - هر پیام فقط یک بار"""
        ...
        # فقط ۵ پیام آخر رو بگیر (برای تشخیص همزمان)
        last_messages = all_messages[-5:] if len(all_messages) >= 5 else all_messages
        
        # لیست پیام‌های جدید (که قبلاً پردازش نشدن)
        new_messages = []
        ...
        for msg in reversed(last_messages):
            ...
            msg_sender, msg_content = msg_parts
            msg_sender = msg_sender.strip()
            msg_content = msg_content.strip()
            
            # فقط پیام‌های همین کاربر
            if msg_sender != sender:
                continue
            
            # ===== ایجاد شناسه یکتا با استفاده از تاریخچه ارسال =====
            import time
            current_time = time.time()
            
            # بررسی تاریخچه این کاربر
            user_history = s._send_history.get(msg_sender, [])
            
            # ===== بررسی اینکه آیا این پیام قبلاً پردازش شده =====
            already_processed = False
            msg_unique_id = None
            
            # بررسی ۲۰ پیام آخر تاریخچه این کاربر
            for hist_content, hist_time, hist_processed in user_history[-20:]:
                if hist_content == msg_content and hist_processed:
                    # اگر قبلاً پردازش شده
                    already_processed = True
                    msg_unique_id = f"{msg_sender}:{msg_content}:{hist_time}"
                    print(f"⏭️ Already processed: {msg_content} at {hist_time:.3f}")
                    break
            
            # اگر قبلاً پردازش شده بود، رد کن
            if already_processed:
                continue
            
            # ===== اینجا پیام جدید یا تکراری ولی پردازش نشده =====
            # استخراج کد و target_id
            code, target_id = s._extract_code_and_target(msg_content, anti_cmds)
            ...
            if str(target_id) == str(owner_client_id):
                # ایجاد شناسه یکتا با زمان فعلی
                msg_unique_id = f"{msg_sender}:{msg_content}:{current_time}"
                ...
                new_messages.append({
                    ...
                    'code': code,
                    'target_id': target_id,
                    'unique_id': msg_unique_id,
                    'timestamp': current_time
                })
                
                # ===== ثبت در تاریخچه به عنوان پردازش نشده (تا بعد از پردازش) =====
                if msg_sender not in s._send_history:
                    s._send_history[msg_sender] = []
                s._send_history[msg_sender].append((msg_content, current_time, False))  # False = پردازش نشده
                
                # محدود کردن تاریخچه به ۲۰ آیتم
                if len(s._send_history[msg_sender]) > 20:
                    s._send_history[msg_sender] = s._send_history[msg_sender][-15:]
        
        # اگر پیام جدیدی پیدا نشد
        if not new_messages:
            return False
        
        # ===== پردازش پیام‌های جدید (حداکثر ۳ تا همزمان) =====
        triggered = False
        processed_count = 0
        
        for msg_data in new_messages[:3]:  # حداکثر ۳ تا
            code = msg_data['code']
            ...
            # ===== بروزرسانی تاریخچه: علامتگذاری به عنوان پردازش شده =====
            if hasattr(s, '_send_history') and msg_sender in s._send_history:
                # پیدا کردن آخرین ورودی با همین محتوا و timestamp
                for i, (hist_content, hist_time, hist_processed) in enumerate(s._send_history[msg_sender]):
                    if hist_content == msg_content and abs(hist_time - timestamp) < 0.01:
                        s._send_history[msg_sender][i] = (hist_content, hist_time, True)  # True = پردازش شده
                        print(f"✅ Marked as processed: {msg_content} at {hist_time:.3f}")
                        break
            
            # ===== ذخیره در کش =====
            if not hasattr(s, '_processed_anti_cache'):
                s._processed_anti_cache = {}
            
            if msg_sender not in s._processed_anti_cache:
                s._processed_anti_cache[msg_sender] = []
            
            s._processed_anti_cache[msg_sender].append(unique_id)
            
            # محدود کردن لیست هر کاربر به ۳۰ آیتم
            if len(s._processed_anti_cache[msg_sender]) > 30:
                s._processed_anti_cache[msg_sender] = s._processed_anti_cache[msg_sender][-20:]
            
            print(f"✅ Anti-code triggered: {msg_sender} -> {code} -> {response} (at {timestamp:.3f})")
            print(f"📊 Cache size for {msg_sender}: {len(s._processed_anti_cache[msg_sender])}")
            
            if delay > 0:
                def send_delayed():
                    s.safe_send_message(response, msg_sender)
                    push(f'🛡️ Anti code triggered for {msg_sender}: {response}', color=(1, 0.5, 0.5))
    ...
    def ear(s):
        ...
        # ===== پاکسازی تاریخچه و کش قدیمی هر ۵ بار =====
        if not hasattr(s, '_clean_counter'):
            s._clean_counter = 0
        s._clean_counter += 1
        
        if s._clean_counter >= 5:  # هر ۵ بار (حدود ۱.۵ ثانیه)
            s._clean_old_history()
            s._clean_old_cache()
            s._clean_counter = 0
        ...
        # پاک کردن لیست پردازش‌های قبلی وقتی کاربر عوض میشه
        if hasattr(s, '_last_sender') and s._last_sender != sender:
            print(f"🔄 Sender changed from {s._last_sender} to {sender}")
        s._last_sender = sender
