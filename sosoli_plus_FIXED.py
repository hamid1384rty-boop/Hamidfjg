...
class MainPanel:
    ...
    def _build(self, data):
        ...
        h = 560 if has_subscription else 500
        ...
        scroll = sw(parent=w, size=(410, h - 320), position=(10, 55),
        ...
        if not has_subscription:
            if remaining > 0:
                SubUI.bw(
                    parent=column, label='🔗 اتصال به اشتراک',
                ...
                tw(parent=column, text='⛔ ظرفیت تکمیل است!',
...
class ConnectPanel:
    def __init__(self, source):
        ...
        w = self.w = SubUI.cw(source=source, size=(420, 300))

        tw(parent=w, text='🔗 اتصال به اشتراک', scale=1.2,
           position=(210, 270), h_align='center', color=(0.8, 0.6, 0.2))
        tw(parent=w, text='🆔 آیدی شما (خودکار): %s' % self.account_id,
           position=(20, 240), scale=0.75, color=(0.6, 0.8, 1), maxwidth=380)
        tw(parent=w, text='اسم خاص خود را وارد کنید:',
           position=(20, 205), scale=0.8, color=(0.8, 0.8, 1))

        self.special_input = tw(
            parent=w, maxwidth=250, size=(250, 32), editable=True,
            v_align='center', color=(0.75, 0.75, 0.75), position=(20, 168),
            allow_clear_button=False, text=GameDetector.get_special_name()
        )

        SubUI.bw(
            parent=w, label='✅ اتصال', size=(110, 35), position=(290, 165),
            on_activate_call=CallStrict(self._connect), color=(0.1, 0.4, 0.1)
        )

        tw(parent=w, text='⚠️ بعد از اتصال، اشتراک به نام شما ثبت می‌شود',
           position=(210, 120), h_align='center', scale=0.65, color=(0.8, 0.6, 0.3))
        tw(parent=w, text='⚠️ و بصورت تصادفی به یک اشتراک خالی وصل می‌شوید',
           position=(210, 95), h_align='center', scale=0.65, color=(0.8, 0.6, 0.3))
    ...
    def _on_connect(self, result):
        if isinstance(result, dict) and result.get('success'):
            ...
            SubUI.ok()
            push('✅ به "%s" متصل شدید!' % sub_name, color=(0, 1, 0))
