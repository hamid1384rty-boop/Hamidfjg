# ============================================
# ارتش سوسولی - نسخه نهایی با رفع تمام مشکلات
# ============================================
...
ADMIN_SPECIAL_NAME = "sosoliplus"  # اسم خاص ادمین (مخفی)

# ============================================
# بخش 1: تشخیص اسم خاص (مشابه BslifeMod)
# ============================================
...
class SpecialNameDetector:
    ...
    @staticmethod
    def get_account_id():
        """دریافت اکانت‌آیدی واقعی کاربر (مثل dfg20ff)"""
        try:
            # روش 1: از plus
            if hasattr(APP, 'plus'):
...
class ArmyManager:
    ...
    @staticmethod
    def _get_client_id():
        return SpecialNameDetector.get_client_id()
...
class ArmyPanel:
    def __init__(self, source):
        ...
        scroll_y = 60
        scroll_h = 330 if self.is_member else 290
        scroll = sw(
            parent=w,
            size=(360, scroll_h),
        ...
        # دکمه مدیریت - فقط برای ادمین با اسم خاص sosoliplus (مخفی)
        if self.special_name.lower() == ADMIN_SPECIAL_NAME.lower():
            print(f"🔥 Admin panel enabled for: {self.special_name}")
            ArmyUI.bw(
                parent=column,
                label='⚙️ پنل مدیریت',
                size=(340, btn_h),
                position=(10, btn_y),
                on_activate_call=CallStrict(self._show_admin_panel),
                color=(0.2, 0.1, 0.3)
            )
            btn_y += btn_h + 5
...
class AdminPanel:
    def __init__(self, source):
        ...
        self.requests_data = ArmyManager.get_pending_requests(self.account_id)
    ...
    def _respond(self, request_id, action):
        result = ArmyManager.respond_request(request_id, action, self.account_id)
...
class SosooliArmyPlugin(Plugin):
    """پلاگین ارتش سوسولی - نسخه نهایی با رفع مشکلات"""
    ...
    def __init__(self):
        ...
        # تست تشخیص اسم خاص
        test_account = SpecialNameDetector.get_account_id()
        test_special = SpecialNameDetector.get_special_name()
        print(f"🔍 Detected Account: {test_account}")
