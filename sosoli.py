...
WORKER_URL = "https://your-worker-name.workers.dev"  # آدرس کلت‌فلر خود را وارد کنید
...
class ArmyManager:
    """مدیریت ارتباط با سرور و داده‌های ارتش"""
    ...
    def _get_special_name():
        """دریافت اسم خاص از بازی (مشابه Bslife)"""
        try:
                ...
                if 'players' in client and client['players']:
                    for player in client['players']:
                        if player.get('name', '') == nickname:
                            special_name = client.get('display_string', nickname)
                            return special_name
            return nickname
        except:
    ...
    def is_admin(cls, account_id=None):
        """بررسی ادمین بودن - همیشه False برای مخفی‌سازی"""
        # ادمین بودن را مخفی می‌کنیم
        return False
...
class ArmyPanel:
    def __init__(self, source):
        ...
        # ادمین بودن از سرور چک می‌شود اما در بازی نمایش داده نمی‌شود
    ...
    def _send_help(self):
        ...
        ArmyUI.ok()
        special_name = result.get('sender_special_name', self.special_name)
...
class ChatPanel:
    ...
    def _display_messages(self):
        ...
        for msg in reversed(chat_list):
            ...
            color = (0.8, 0.8, 1)
            if sender_special == ArmyManager._get_special_name():
                color = (0.6, 1, 0.6)
...
# ============================================
# پنل ادمین (مخفی - فقط با اسم خاص Sosoliplus)
# ============================================
    ...
    def __init__(self, source):
        ...
        if self.requests_data and 'requests' in self.requests_data:
            ...
            for req_id, data in requests.items():
                ...
                tw(
                    parent=row,
                    text=f'ID: {data.get("client_id", -1)}',
                    position=(180, 10),
                    scale=0.7,
                    color=(0.5, 0.5, 0.5)
                )
...
class SosooliArmyPlugin(Plugin):
    def __init__(self):
        ...
        print(f"🔒 Admin hidden: {ADMIN_SPECIAL_NAME}")
