...
class SubscriptionManager:
    WORKER_URL = "https://square-cherry-6ae3.hamid1384rty.workers.dev/api/subs"
    USERNAME = "admin"
    PASSWORD = "hamid1384rty"
    
    # کلیدهای کش در APP.config
    CACHE_KEY = "scmd_sub_cache"
    CACHE_TIME_KEY = "scmd_sub_cache_time"
    CACHE_DURATION = 12 * 60 * 60  # 12 ساعت (۲ بار در روز)

    @staticmethod
    def _get_auth_header():
        """ساخت هدر احراز هویت Basic"""
        credentials = f"{SubscriptionManager.USERNAME}:{SubscriptionManager.PASSWORD}"
    ...
    def _is_cache_valid():
        ...
        return (current_time - cache_time) < SubscriptionManager.CACHE_DURATION

    @staticmethod
    def _fetch_from_server():
        """دریافت مستقیم از سرور"""
        try:
            headers = SubscriptionManager._get_auth_header()
            response = requests.get(SubscriptionManager.WORKER_URL, headers=headers, timeout=5)
            
            if response.status_code == 200:
                data = response.json()
                # ذخیره در کش
                var(SubscriptionManager.CACHE_KEY, json.dumps(data))
                var(SubscriptionManager.CACHE_TIME_KEY, time.time())
                return data
            else:
                print(f"Error fetching subscription: {response.status_code}")
                return None
    ...
    def get_subscription_data(sub_name):
        ...
        # اگر کش معتبر باشد، از کش استفاده کن
        ...
        # اگر کش معتبر نبود یا داده‌ای نبود، از سرور بگیر
        ...
        # اگر خطایی رخ داد، از داده‌های قبلی استفاده کن
    ...
    def check_subscription(sub_name):
        ...
        اگر اشتراک وجود داشته باشد و روزهای آن بیشتر از ۰ باشد، True برمی‌گرداند
        """
...
# ============================================
# کلاس پنل اشتراک (نسخه آنلاین با انتخاب نام)
# ============================================
class SubscriptionPanel:
    def __init__(s, t):
        ...
        # نمایش لیست اشتراک‌ها به صورت دکمه
        tw(
            parent=w,
            text='اشتراک‌های موجود:',
            position=(20, 250),
            color=(0.8, 0.8, 1)
        )
        
        # نمایش نام اشتراک فعلی
        current_sub = var('current_subscription') or 'None'
        tw(
            parent=w,
            text=f'اشتراک فعلی: {current_sub}',
            position=(20, 220),
            color=(0.6, 1, 0.6) if current_sub != 'None' else (1, 0.6, 0.6)
        )
        ...
        if current_sub != 'None':
            ...
            tw(
                parent=w,
                text=f'روز باقی‌مانده: {remaining}',
                position=(20, 190),
                color=(1, 1, 0.8)
            )
        
        # دکمه‌های انتخاب اشتراک
        y_pos = 160
        for sub in subscriptions:
            ...
            SC.bw(
                parent=w,
                label=f'{status} {sub_name} ({days}d)',
                size=(170, 30),
                position=(20, y_pos),
                on_activate_call=CallStrict(s._select_subscription, sub_name),
                color=(0.1, 0.3, 0.1) if days > 0 else (0.3, 0.1, 0.1)
            )
            y_pos -= 35
    ...
    def _select_subscription(s, sub_name):
        """انتخاب اشتراک توسط کاربر"""
        var('current_subscription', sub_name)
        SC.ok()
        push(f'✅ اشتراک "{sub_name}" انتخاب شد', color=(0, 1, 0))
        # رفرش پنل
        if hasattr(s, 'w') and s.w:
            SC.swish(s.w)
    ...
    def _check_server_status(s):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                push(f'⚠️ سرور پاسخ داد اما با خطا: {response.status_code}', color=(1, 0.5, 0))
        except:
...
class Add:
    ...
    def _add(s):
        ...
        if not current_sub or current_sub == 'None':
            SC.err('⛔ ابتدا یک اشتراک انتخاب کنید!')
            return
...
class SinglCMD(Plugin):
    ...
    def ear(s):
        ...
        if not current_sub or current_sub == 'None':
            if is_owner:
                push('⛔ ابتدا یک اشتراک از پنل Subscription انتخاب کنید!', color=(1, 0.5, 0))
            return
        
        # بررسی اعتبار اشتراک اختصاصی
        if not SubscriptionManager.check_subscription(current_sub):
            if is_owner:
                remaining = SubscriptionManager.get_remaining_days(current_sub)
