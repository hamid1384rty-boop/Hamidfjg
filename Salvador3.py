...
class GameDetector:
    # ذخیره اسم ثبت‌نامی کاربر
    _registered_name = None
    
    @staticmethod
    def set_registered_name(name):
        """ذخیره اسمی که کاربر هنگام ثبت‌نام وارد کرده"""
        GameDetector._registered_name = name
    
    @staticmethod
    def get_registered_name():
        """دریافت اسم ثبت‌نامی کاربر"""
        if GameDetector._registered_name:
            return GameDetector._registered_name
        return GameDetector.get_special_name()

    # --- تشخیص نام سرور (کش‌شده + به‌روزرسانی خودکار در ترد اصلی) ---
    _server_name_cache = None
    _server_name_cache_time = 0
    SERVER_NAME_CACHE_TTL = 300  # 5 دقیقه

    @staticmethod
    def _detect_server_name():
        """
        تشخیص نام سرور/پارتی فعلی - فقط از ترد اصلی (منطقی) صدا بزنید.
        نکته مهم API 9: get_connection_to_host_info یک آبجکت برمی‌گرداند
        (نه dict)، پس info.get('name') خطا می‌داد و همه روش‌ها بی‌فایده
        می‌شدند. اینجا هم حالت dict هم attribute چک می‌شود.
        """
        import bascenev1 as bs

        # روش 1: اطلاعات اتصال به هاست (وقتی به سرور دیگری وصلیم) - معتبرترین
        try:
        ...
        # روش 2: API خام داخلی (در بعضی نسخه‌ها/مادها فقط این در دسترس است)
        try:
            import _bascenev1
            raw = _bascenev1.get_connection_to_host_info_2()
            if raw is not None:
                try:
        ...
        # روش 3: وقتی خودمان هاستیم => نام پارتی از کانفیگ
        try:
            for key in ('Party Name', 'party_name'):
                name = babase.app.config.get(key)
                if name and str(name).strip() and str(name).lower() != 'none':
                    return str(name)
        except Exception:
            pass
        ...
        try:
            ...
            if session is not None:
                get_name = getattr(session, 'get_name', None)
        ...
        # روش 5: اسم هاست (اولین کلاینت رستر) به جای «نامشخص»
        try:
            roster = get_game_roster()
            if roster:
                host = roster[0]
                if isinstance(host, dict):
                    players = host.get('players') or []
                    if players and players[0].get('name'):
                        return str(players[0]['name'])
                    if host.get('display_string'):
                        return str(host['display_string'])
        except Exception:
            pass
    ...
    @staticmethod
    def _refresh_server_name():
        """به‌روزرسانی کش نام سرور - فقط از ترد اصلی صدا بزنید"""
        try:
            name = GameDetector._detect_server_name()
            if name and name != 'نامشخص':
                GameDetector._server_name_cache = name
                GameDetector._server_name_cache_time = time.time()
        except Exception:
            pass
    ...
    def get_server_name():
        """
        دریافت نام سرور - از هر تردی قابل فراخوانی است.
        اول تشخیص زنده را امتحان می‌کند؛ اگر ممکن نبود یا خطا داد،
        از کشِ به‌روزشده با تایمر استفاده می‌کند تا دیگر «نامشخص» نیفتد.
        """
        try:
            name = GameDetector._detect_server_name()
            if name and name != 'نامشخص':
            ...
            pass
        cached = GameDetector._server_name_cache
        if cached and (time.time() - GameDetector._server_name_cache_time) < GameDetector.SERVER_NAME_CACHE_TTL:
            return cached
        return 'نامشخص'

    @staticmethod
    def parse_help_message(message):
        """استخراج (اسم فرستنده, نام سرورِ فرستنده) از متن پیام درخواست کمک"""
        try:
            text = str(message)
            if 'درخواست کمک' not in text:
                return None, None
            head = text.split('درخواست کمک')[0].strip()
            parts = head.split(' | ')
            if len(parts) >= 2:
                sender = parts[0].strip()
                server = ' | '.join(parts[1:]).strip()
                if sender and server:
...
class SubscriptionManager:
    ...
    @classmethod
    def send_help_request(cls, message, account_id=None):
        """ارسال درخواست کمک با فیلد is_help"""
        if not account_id or account_id == 'Unknown':
            account_id = GameDetector.get_account_id()
        return cls._make_request(
            '/api/subs/chat/send',
            {'account_id': account_id, 'message': message, 'is_help': True},
            'POST',
        )
...
class SubUI:
    ...
    @staticmethod
    def big_green_message(text):
        """نمایش پیام سبز بزرگ در مرکز صفحه"""
        try:
            push(str(text), color=(0, 1, 0), scale=1.5)
        except Exception:
            try:
                push(str(text), color=(0, 1, 0))
            except Exception:
                pass
...
class MainPanel:
    ...
    def _send_help_request(self):
        """ارسال درخواست کمک به همه اعضای اشتراک با اسم ثبت‌نامی فرد"""
        # استفاده از اسم ثبت‌نامی فرد به جای sosoliplus
        registered_name = GameDetector.get_registered_name()
        if not registered_name or registered_name == 'Unknown':
            registered_name = GameDetector.get_special_name()
        
        push('🆘 درخواست کمک ارسال شد...', color=(1, 1, 0))
        # تشخیص نام سرور در لحظه
        server_name = GameDetector.get_server_name()
        message = f'{registered_name} | {server_name} درخواست کمک 🙏'
        Net.run(
            CallPartial(SubscriptionManager.send_help_request, message, GameDetector.get_account_id()),
            self._on_help_sent
        )
    ...
    def _on_help_sent(self, result):
        if isinstance(result, dict) and result.get('success'):
            SubUI.ok()
            # نمایش پیام سبز بزرگ برای خود فرستنده
            SubUI.big_green_message('✅ درخواست کمک با موفقیت ارسال شد!')
...
class ConnectPanel:
    ...
    def _connect(self):
        ...
        # ذخیره اسم ثبت‌نامی برای استفاده در درخواست کمک
        GameDetector.set_registered_name(special_name)
...
class ChatPanel:
    # ⚠️ پولینگ هوشمند: وقتی چت فعاله سریع رفرش می‌شه، وقتی ساکته آروم‌تر
    # می‌شه تا مصرف درخواست‌های سرور (و در نتیجه ریسک برخورد به سقف روزانه‌ی
    # KV) کمتر بشه. هیچ رفتار یا قابلیتی حذف نشده، فقط فاصله‌ی رفرش پویا شده.
    FAST_REFRESH = 3.0
    SLOW_REFRESH = 15.0
    IDLE_TICKS_BEFORE_SLOWDOWN = 4
    
    # ⚠️ برای نمایش پیام سبز درخواست کمک حتی وقتی چت بسته است
    HELP_CHECK_INTERVAL = 30.0
    HELP_MESSAGE_MAX_AGE = 60.0  # فقط پیام‌های کمتر از ۱ دقیقه نمایش داده بشن
    _last_help_timestamp = 0
    _help_timer = None
    _help_checker_running = False
    ...
    def __init__(self, source, data=None):
        ...
        self._timer = None
        self.chat_data = {'chat': []}
        self._refresh_interval = ChatPanel.FAST_REFRESH
        self._idle_ticks = 0
        self._last_help_shown = False
    ...
    def _build_inner(self, data):
        ...
        # تایمر به‌روزرسانی خودکار (پویا: سریع وقتی فعال، آروم وقتی ساکت)
    ...
    def _schedule_next_refresh(self):
        if not self._alive:
            return
        try:
            self._timer = AppTimer(self._refresh_interval, CallStrict(self._refresh_tick))
        except Exception:
            self._timer = None
    ...
    def _render_messages(self):
        ...
        for msg in chat_list:
            ...
            # تشخیص پیام درخواست کمک (بدون نیاز به فیلد is_help)
            is_help = 'درخواست کمک' in message
            
            # اگر پیام درخواست کمک است، پیام سبز بزرگ نمایش بده (فقط یک بار)
            if is_help and not self._last_help_shown:
                # ⚠️ سرورِ خودِ فرستنده را از داخل متن پیام بخوان، نه سرورِ ما
                _p_sender, _p_server = GameDetector.parse_help_message(message)
                if _p_sender:
                    sender = _p_sender
                server_name = _p_server or GameDetector.get_server_name()
                SubUI.big_green_message('🆘 درخواست کمک: %s | %s' % (sender, server_name))
                self._last_help_shown = True
                # بعد از ۵ ثانیه دوباره اجازه بده
                try:
                    AppTimer(5.0, CallStrict(self._reset_help_shown))
                except Exception:
                    pass
    ...
    def _send_help_request(self):
        # ارسال درخواست کمک از داخل پنل چت با اسم ثبت‌نامی فرد
        registered_name = GameDetector.get_registered_name()
        if not registered_name or registered_name == 'Unknown':
            registered_name = GameDetector.get_special_name()
        ...
        # تشخیص نام سرور در لحظه
        server_name = GameDetector.get_server_name()
        help_message = f'{registered_name} | {server_name} درخواست کمک 🙏'
        self._busy = True
        Net.run(
            CallPartial(SubscriptionManager.send_help_request, help_message, self.account_id),
            self._on_help_sent
        )
    ...
    def _on_help_sent(self, result):
        ...
        if isinstance(result, dict) and result.get('success'):
            ...
            if not registered_name or registered_name == 'Unknown':
                registered_name = GameDetector.get_special_name()
            SubUI.big_green_message('🆘 درخواست کمک: %s | %s' % (registered_name, server_name))
            push('🆘 همه اعضای اشتراک از درخواست شما مطلع شدند', color=(1, 0.8, 0))
            self._refresh_interval = ChatPanel.FAST_REFRESH
            self._idle_ticks = 0
    ...
    def _on_sent(self, result):
        ...
        if isinstance(result, dict) and result.get('success'):
            ...
            try:
                AppTimer(1.0, CallStrict(self._manual_refresh))
            except Exception:
                self._manual_refresh() # اگر تایمر خطا داد، مستقیم اجرا کن
        else:
            msg = result.get('error', 'خطا در ارسال') if isinstance(result, dict) else 'خطا در ارسال'
            SubUI.err('❌ ' + str(msg))

    def _manual_refresh(self):
        # رفرش فوری و یک‌باره (مثلاً بلافاصله بعد از ارسال پیام) — بدون
        # زمان‌بندی تایمر جدید، تا با حلقه‌ی اصلی تداخل/تکرار پیدا نکنه.
    ...
    def _on_manual_refresh(self, result):
        self._busy = False
        if not self._alive:
            return
    ...
    def _refresh_tick(self):
        # این متد فقط برای حلقه‌ی خودزمان‌بند اصلی استفاده می‌شود؛ در انتها
        # خودش تایمر بعدی را زمان‌بندی می‌کند (_on_refresh).
        if not self._alive or self._busy:
            return
        self._busy = True
        Net.run(
    ...
    def _apply_refresh_result(self, result):
        """پردازش نتیجه‌ی دریافتی از سرور و برگرداندن اینکه آیا پیام جدیدی آمده یا نه."""
        changed = False
        if isinstance(result, dict) and 'chat' in result:
    ...
    def _on_refresh(self, result):
        ...
        changed = self._apply_refresh_result(result)

        # ⚠️ پولینگ هوشمند: اگر پیام جدیدی اومده، فاصله رو سریع نگه دار.
        # اگر مدتی (چند تیک پشت‌سرهم) هیچ تغییری نبوده، آروم‌آروم فاصله رو
        # زیاد کن تا فشار کمتری روی سرور بیفته. به‌محض اومدن پیام جدید یا
        # ارسال پیام توسط خود کاربر، دوباره سریع می‌شه.
        if changed:
            self._idle_ticks = 0
            self._refresh_interval = ChatPanel.FAST_REFRESH
        else:
            self._idle_ticks += 1
            if self._idle_ticks >= ChatPanel.IDLE_TICKS_BEFORE_SLOWDOWN:
                self._refresh_interval = ChatPanel.SLOW_REFRESH
...
class SosooliPlusPlugin(Plugin):
    ...
    def __init__(self):
        ...
        # ⚠️ شروع تایمر چک درخواست کمک از همان ابتدا - حتی وقتی چت باز نشده
        self._start_global_help_checker()
        
        # به‌روزرسانی خودکار کش نام سرور در ترد اصلی تا هیچ‌وقت «نامشخص» نیفتد
        try:
            AppTimer(3.0, CallStrict(GameDetector._refresh_server_name), repeat=True)
            print('🔑 %s - Server name auto-detector started!' % SYSTEM_NAME)
        except Exception as exc:
            print('🔑 %s - server name timer error: %s' % (SYSTEM_NAME, exc))

        self._start_status_updater()
        self._inject_party_button()

    def _start_global_help_checker(self):
        """شروع تایمر چک درخواست کمک در پس‌زمینه - از همان ابتدا"""
        try:
            if not ChatPanel._help_checker_running:
                ChatPanel._help_checker_running = True
                ChatPanel._help_timer = AppTimer(ChatPanel.HELP_CHECK_INTERVAL, CallStrict(self._global_help_check_tick), repeat=True)
                print('🔑 %s - Global help checker started!' % SYSTEM_NAME)
        except Exception as exc:
            print('🔑 %s - Global help checker error: %s' % (SYSTEM_NAME, exc))

    def _global_help_check_tick(self):
        """چک کردن پیام‌های جدید برای درخواست کمک - در پس‌زمینه - بدون نیاز به چت"""
        def _work():
            try:
                ...
                result = SubscriptionManager.get_chat_messages(aid, limit=10)
                if isinstance(result, dict) and 'chat' in result:
                    chat_list = result.get('chat', [])
                    current_time = time.time() * 1000  # تبدیل به میلی‌ثانیه
                    for msg in reversed(chat_list):
                        message = msg.get('message', '')
                        if 'درخواست کمک' in message:
                            ts = msg.get('timestamp', 0)
                            # فیلتر: فقط پیام‌های کمتر از ۱ دقیقه قبل
                            if current_time - ts <= ChatPanel.HELP_MESSAGE_MAX_AGE * 1000:
                                if ts > ChatPanel._last_help_timestamp:
                                    ChatPanel._last_help_timestamp = ts

                                    # ⚠️ اسم و سرورِ فرستنده را از خودِ متن پیام بخوان
                                    p_sender, p_server = GameDetector.parse_help_message(message)
                                    ...
                                    # اگر سرور داخل متن نبود، از تشخیص محلی/کش استفاده کن
                                    server_name = p_server or GameDetector.get_server_name()

                                    # نمایش پیام سبز با نام سرور
                                    pushcall(
                                        CallStrict(SubUI.big_green_message, '🆘 درخواست کمک: %s | %s' % (sender, server_name)),
