...
class MainPanel:
    ...
    def _send_help_request(self):
        """ارسال درخواست کمک به همه اعضای اشتراک با اسم خاص فرد"""
        # استفاده از اسم خاص فرد به جای sosoliplus
        special_name = GameDetector.get_special_name()
        if not special_name or special_name == 'Unknown':
            special_name = GameDetector.get_nickname()
        
        push('🆘 درخواست کمک ارسال شد...', color=(1, 1, 0))
        server_name = GameDetector.get_server_name()
        message = f'{special_name} در سرور {server_name} درخواست کمک 🙏'
        Net.run(
...
class ChatPanel:
    ...
    def _send_help_request(self):
        # ارسال درخواست کمک از داخل پنل چت با اسم خاص فرد
        special_name = GameDetector.get_special_name()
        if not special_name or special_name == 'Unknown':
            special_name = GameDetector.get_nickname()
        
        server_name = GameDetector.get_server_name()
        help_message = f'{special_name} در سرور {server_name} درخواست کمک 🙏'
        self._busy = True
        Net.run(
